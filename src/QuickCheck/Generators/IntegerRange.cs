...
/// <summary>
/// Maps one choice onto an integer in [min, max], laid out so that choice 0 is the value nearest
/// zero and increasing choices alternate outwards (0, +1, -1, +2, -2, …) until one bound is
/// exhausted. Shrinking a choice therefore shrinks the magnitude of the integer. A struct, so a
/// generator whose bounds vary per draw can build one without allocating.
/// </summary>
internal readonly struct IntegerRange<T> where T : IBinaryInteger<T>
{
    ...
    private readonly ulong _minChoice;
    private readonly ulong _maxBoundChoice;

    public IntegerRange(T min, T max)
    {
        if (min > max)
        {
        ...
        _minChoice = ToChoice(_below, above: false);
        _maxBoundChoice = ToChoice(_above, above: true);
    }
