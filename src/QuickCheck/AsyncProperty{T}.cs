...
/// value a <see cref="Generator{T}"/> produces, and for every value pinned with
/// <see cref="Example"/>.
/// </summary>
...
public sealed class AsyncProperty<T>
{
    ...
    private AsyncProperty(PropertyRunner<T> runner, T[] examples)
    {
        _runner = runner;
        _examples = examples;
    }
    ...
    /// <see cref="CheckAsync"/> throws rather than leave the pins unchecked.
    /// </para>
    /// </remarks>
    public AsyncProperty<T> Example(T value) => new(_runner, [.. _examples, value]);
    ...
    public Task<PropertyResult<T>> CheckAsync(
        CheckOptions? options = null, CancellationToken cancellationToken = default) =>
        _runner.CheckAsync(options ?? CheckOptions.Default, _examples, cancellationToken).AsTask();
}
